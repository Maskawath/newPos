...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	@if(!empty($is_direct_sell))
		...
		@endphp
	        ...
	        <input type="text" name="products[{{$row_count}}][last_purchased_price]" class="form-control pos_purchase_price input_number mousetrap" value="{{@num_format($purchase_price)}}" disabled>
	    </td>
	     <td @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
	        <input type="text" name="products[{{$row_count}}][profit_percent]" class="form-control pos_profit_percent input_number mousetrap" value="{{@num_format($profit_percent)}}">
	        <br>
	        <label for="custom_profit_percent">Custom Profit Percent</label>
	        <input type="text" name="products[{{$row_count}}][custom_profit_percent]" class="form-control custom_profit_percent input_number mousetrap" value="{{@num_format($custom_profit_percent)}}">
	   </td>
		<td @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
